                ...
                <div class="panel">
                    <div style="    background-color: linear-gradient(to right, #2980b9, #2c3e50);;" class="panel-heading">
                        <div class="row">
                            <div class="col col-sm-3 col-xs-12">
                                <h4 class="title"> <span>List</span></h4>
                            </div>
                        </div>
                    </div>
                    ...
                    <th>Action</th>
                            ...
                            <?php  foreach($horairesDATA as $horaire) : ?>
                                <tr>
                                <td><?= $horaire->getIdHoraire(); ?></td>
                        <td><?= $horaire->getIdRoute(); ?></td>
                        <td><?= $horaire->getIdBus(); ?></td>
                        <td><?= $horaire->getDate(); ?></td>
                        <td><?= $horaire->getHeureDepart(); ?></td>
                        <td><?= $horaire->getHeureArrivee(); ?></td>
                        <td><?= $horaire->getSiegesDispo(); ?></td>
                       
                                    <td>
                                        <ul class='action-list'>
                                        <li><a href="index.php?action=ShowAddHoraire&id=<?= $horaire->getIdHoraire() ?>" data-tip='add'><i class='fa fa-plus'></i></a></li>
                                        <li><a href="index.php?action=showUpdateHoraire&id=<?= $horaire->getIdHoraire() ?>" data-tip='edit'><i class='fa fa-edit'></i></a></li>
<li><a href="index.php?action=deleteHoraire&id=<?= $horaire->getIdHoraire() ?>" data-tip='delete'><i class='fa fa-trash'></i></a></li>
